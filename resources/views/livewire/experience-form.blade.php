                    ...
                    <ul role="list" class="grid grid-cols-1 gap-5 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3 p-4">
                        @foreach ($experiences as $key => $experience)
                            @if ($key >= 10)
                                @break
                            @endif
                                <li x-data="{ settings: false }" @click.away="settings = false" @close.stop="settings = false" class="{{ (auth()->user()->dark_mode) ? 'bg-[#4FAE9D]' : 'bg-gray-300' }} col-span-1 shadow-md rounded-md relative flex items-end h-[250px] overflow-hidden group" role="option">
