    ...
    @if(count($currentUser->projects) >= 1)
        <div class="mt-6 space-y-4">
            @foreach($currentUser->projects as $project)
                <div aria-roledescription="list of all projects" class="{{ (auth()->user()->dark_mode) ? 'bg-[#384049] sm:hover:bg-[#2a2f36]' : 'bg-white' }} group shadow-md sm:hover:shadow-sm rounded-lg sm:transition-all duration-300 ease-linear">
                    <div class="px-4 py-4 sm:px-6">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="ml-2 flex-shrink-0 flex items-center">
                                ...
                                <button wire:click.prevent="editProject({{ $project->id }})" @click="edit = true"  type="button" class="{{ (auth()->user()->dark_mode) ? 'text-white sm:hover:text-[#E96C97]' : 'text-[#733d91] sm:hover:text-indigo-400' }} flex font-semibold opacity-50 sm:group-hover:opacity-100 sm:transition-opacity">
                                    Edit
                                </button>
                            </div>
                        </div>
                        <div class="mt-2 sm:flex sm:justify-between">
                            <div class="sm:flex">
                                <a href="{{ $project->links['github'] }}" target="_blank" class="{{ (auth()->user()->dark_mode) ? 'text-white sm:hover:text-[#E96C97]' : 'text-indigo-700 sm:text-gray-500 sm:hover:text-indigo-400' }} opacity-50 sm:group-hover:opacity-100 flex items-center text-sm sm:transition-opacity">
                                ...
                                <a href="{{ $project->links['code'] }}" target="_blank" class="{{ (auth()->user()->dark_mode) ? 'text-white sm:hover:text-[#E96C97]' : 'text-indigo-700 sm:text-gray-500 sm:hover:text-indigo-400' }} opacity-50 sm:group-hover:opacity-100 mt-2 flex items-center text-sm sm:mt-0 sm:ml-6 sm:transition-opacity">
