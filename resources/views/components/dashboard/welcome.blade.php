    ...
    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
        <div class="flex flex-col justify-center space-y-4 rounded-lg">
            @if($currentUser->profile->settings['track_views'])
                <div class="h-full flex px-4 py-5 {{ ($currentUser->dark_mode) ? 'bg-[#742fa8]' : 'bg-white' }} overflow-hidden sm:p-6 shadow-md sm:hover:shadow-sm rounded-lg sm:transition-all duration-300 ease-linear">
                    ...
                    <div>
                        <dt class="text-sm font-medium {{ ($currentUser->dark_mode) ? 'text-white' : 'text-gray-500' }} truncate">
                            Total Views
                        </dt>
