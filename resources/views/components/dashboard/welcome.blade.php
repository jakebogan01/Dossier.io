...
<div class="mx-auto px-4 sm:px-6 lg:px-8 mt-8">
    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
        <div class="flex flex-col {{ (!$currentUser->profile->settings['track_likes'] || !$currentUser->profile->settings['track_views']) ?: 'justify-center' }} space-y-4 rounded-lg">
            @if($currentUser->profile->settings['track_views'])
                <div class="{{ (!$currentUser->profile->settings['track_likes'] || !$currentUser->profile->settings['track_views']) ? 'h-[108px]' : 'h-full' }} flex px-4 py-5 {{ ($currentUser->dark_mode) ? 'bg-[#742fa8]' : 'bg-white' }} overflow-hidden sm:p-6 shadow-md sm:hover:shadow-sm rounded-lg sm:transition-all duration-300 ease-linear">
                    <div class="flex items-center h-[60px] {{ ($currentUser->dark_mode) ? 'bg-[#140157]' : 'bg-[#57178D]' }} rounded-md p-4 mr-4">
                    ...
                    <div>
                        <dt class="text-sm font-medium {{ ($currentUser->dark_mode) ? 'text-white' : 'text-gray-500' }} truncate cursor-default">
                            Total Views
                        </dt>
                        <dd class="mt-1 text-3xl font-semibold {{ ($currentUser->dark_mode) ? 'text-white' : 'text-gray-900' }}">
            ...
            <div class="{{ (!$currentUser->profile->settings['track_likes'] || !$currentUser->profile->settings['track_views']) ? 'h-[108px]' : 'h-full' }} flex px-4 py-5 {{ ($currentUser->dark_mode) ? 'bg-[#742fa8]' : 'bg-white' }} overflow-hidden sm:p-6 shadow-md sm:hover:shadow-sm rounded-lg sm:transition-all duration-300 ease-linear">
                <div class="flex items-center h-[60px] {{ ($currentUser->dark_mode) ? 'bg-[#140157]' : 'bg-[#57178D]' }} rounded-md p-3.5 mr-4">
        ...
        <div class="{{ ($currentUser->dark_mode) ? 'bg-[#742fa8]' : 'bg-white' }} rounded-lg p-4 shadow-md sm:hover:shadow-sm rounded-lg sm:transition-all duration-300 ease-linear overflow-hidden">
