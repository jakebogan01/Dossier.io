    ...
    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
        <div class="{{ (!$currentUser->profile->settings['track_likes'] || !$currentUser->profile->settings['track_views']) ?: 'justify-center' }} flex flex-col space-y-4 rounded-lg">
            @if($currentUser->profile->settings['track_views'])
                <div aria-roledescription="total views stat" class="{{ (!$currentUser->profile->settings['track_likes'] || !$currentUser->profile->settings['track_views']) ? 'h-[108px]' : 'h-full' }} {{ ($currentUser->dark_mode) ? 'bg-[#384049] sm:hover:bg-[#2a2f36]' : 'bg-white' }} flex px-4 py-5 overflow-hidden sm:p-6 shadow-md sm:hover:shadow-sm rounded-lg sm:transition-all duration-300 ease-linear">
                    <div class="{{ ($currentUser->dark_mode) ? 'bg-[#4FAE9D]' : 'bg-[#57178D]' }} flex items-center h-[60px] rounded-md p-4 mr-4">
                    ...
                    <div>
                        <dt class="{{ ($currentUser->dark_mode) ? 'text-white' : 'text-gray-500' }} text-sm font-medium truncate cursor-default">
                            Total Views
                        </dt>
                        <dd class="{{ ($currentUser->dark_mode) ? 'text-white' : 'text-gray-900' }} mt-1 text-3xl font-semibold">
            ...
            <div aria-roledescription="portfolio link stat" class="{{ (!$currentUser->profile->settings['track_likes'] || !$currentUser->profile->settings['track_views']) ? 'h-[108px]' : 'h-full' }} {{ ($currentUser->dark_mode) ? 'bg-[#384049] sm:hover:bg-[#2a2f36]' : 'bg-white' }} flex px-4 py-5 overflow-hidden sm:p-6 shadow-md sm:hover:shadow-sm rounded-lg sm:transition-all duration-300 ease-linear">
                <div class="{{ ($currentUser->dark_mode) ? 'bg-[#4FAE9D]' : 'bg-[#57178D]' }} flex items-center h-[60px] rounded-md p-3.5 mr-4">
        ...
        <div aria-roledescription="recent activities" class="{{ ($currentUser->dark_mode) ? 'bg-[#384049] sm:hover:bg-[#2a2f36]' : 'bg-white' }} rounded-lg p-4 shadow-md sm:hover:shadow-sm rounded-lg sm:transition-all duration-300 ease-linear overflow-hidden group">
            <p class="{{ ($currentUser->dark_mode) ? 'text-white' : 'text-[#19077C]' }} font-bold text-sm mb-4" style="font-family: Lato, serif;">
                Recent Activity
            </p>
        ...
        @if($currentUser->profile->settings['track_likes'] && count($currentUser->projects) >= 1)
            <div aria-roledescription="project chart" class="{{ ($currentUser->dark_mode) ? 'bg-[#384049] sm:hover:bg-[#2a2f36]' : 'bg-white' }} rounded-lg pt-4 px-1 shadow-md sm:hover:shadow-sm rounded-lg sm:transition-all duration-300 ease-linear overflow-hidden">
                {!! $barChart->container() !!}
            </div>
        @endif

        @if($currentUser->profile->settings['track_views'] && $currentUser->total_views >= 1)
            <div aria-roledescription="like chart" class="{{ ($currentUser->dark_mode) ? 'bg-[#384049] sm:hover:bg-[#2a2f36]' : 'bg-white' }} rounded-lg pt-4 px-1 shadow-md sm:hover:shadow-sm rounded-lg sm:transition-all duration-300 ease-linear overflow-hidden">
                {!! $areaChart->container() !!}
            </div>
        @endif
    </div>
