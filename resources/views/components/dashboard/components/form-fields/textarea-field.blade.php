...
<div class="{{ $styles }}">
    <div class="relative mt-1">
        ...
        <textarea wire:model.defer="{{ $model }}" id="{{ $model }}" name="{{ $model }}" rows="5" maxlength="{{ $max }}" class="{{ (auth()->user()->dark_mode) ? 'bg-[#262c33] border-transparent focus-within:ring-transparent focus-within:border-transparent text-white' : 'bg-white border-gray-300 focus-within:ring-indigo-600 focus-within:border-indigo-600 text-gray-900' }} shadow-sm mt-1 block w-full sm:text-sm border rounded-md" {{ $required }}></textarea>
    </div>
